     ...
     <div class="row">
         <div class="col-md-12">
             <div class="card">
                 ...
                 <div class="card-body">
                    <form action="{{route('provider.tests.update',$test->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="doctor">Name</label>
                            <input type="text" class="form-control" value="{{ $test->name }}" name="name" placeholder="Enter Name">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="user" placeholder="Enter Name">
                                <option value="{{ $test->patient->id }}" default> {{ $test->patient->name }}</option>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" value="{{ $test->comment }}" name="comment" placeholder="Enter Comment">{{ $test->comment }}</textarea>
                            @error('description')
                            <span class="text-danger">{{$message}}</span>
                            @enderror
                        </div>
